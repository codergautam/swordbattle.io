...
<button onClick={scrollToTarget3} data-selected-badge="event">Event Skins</button>
  ...
  skin?.buyable ? 'Free' : ''
      ...
      {!searchTerm && (
        ...
        <div className='label'>
        <span style={{color: 'yellow'}}>Pi Day Sale!</span><hr></hr>
        <p>Save thousands of gems on the Pi Guy skins, and buy the event-exclusive Pi Day skins before 3/17!</p>
        </div>
        <div className='skins'>
      {Object.values(skins).filter((skinData: any) => {
        const skin = skinData as Skin;
        ...
        <div className="skin-card" key={skin.name}>
          ...
          {/* Get image width from src */}
            ...
            data-selected='skin'
            style={skin.id === 459 ? { transform: 'scale(1.15)' } : undefined}
            />
            <img
            src={basePath + skin.swordFileName}
            alt={skin.name}
