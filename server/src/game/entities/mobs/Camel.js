...
// All Comments are alterations from the Moose mob
class CamelMob extends Entity {
  static defaultDefinition = {
    forbiddenBiomes: [Types.Biome.Safezone, Types.Biome.River],
  };
  constructor(game, objectData) {
    objectData = Object.assign({ size: 125 }, objectData);
    super(game, Types.Entity.Camel, objectData);

    this.shape = Circle.create(0, 0, this.size);
    this.angle = helpers.random(-Math.PI, Math.PI);
    this.coinsDrop = 1200;

    this.movementTimer = new Timer(0, 4, 5); // Longer movement periods
    this.stayTimer = new Timer(3, 3, 5); // Longer rest periods
    this.angryTimer = new Timer(0, 10, 15);
    this.damage = new Property(15); // Reduced damage from 25
    this.isMoving = false;
    this.targetAngle = this.angle;
    this.startAngle = this.angle;

    this.health = new Health(180, 3); // Increased health from 100, regeneration from 1 to 3
    this.speed = new Property(5); // Reduced speed from 8
    this.target = null;
  ...
  update(dt) {
    ...
    this.velocity.scale(0.94); // Slightly less friction for smoother movement
