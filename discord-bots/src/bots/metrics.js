...
function buildDigestEmbed(digest, prior, ccuRec) {
  const p = prior || {};
  const fields = [
    { name: 'DAU', value: fmtNum(digest.dau) + fmtDelta(digest.dau, p.dau), inline: true },
    { name: 'Visits', value: fmtNum(digest.visits) + fmtDelta(digest.visits, p.visits), inline: true },
    { name: 'New players', value: fmtNum(digest.newPlayers) + fmtDelta(digest.newPlayers, p.newPlayers), inline: true },
    { name: 'Returning players', value: fmtNum(digest.returningPlayers) + fmtDelta(digest.returningPlayers, p.returningPlayers), inline: true },
    { name: 'Games played', value: fmtNum(digest.gamesPlayed) + fmtDelta(digest.gamesPlayed, p.gamesPlayed), inline: true },
    ...
    { name: 'New player avg (<7d)', value: fmtMin(digest.newPlayerAvgPlaytimeMin), inline: true },
    ...
    { name: 'Ad revenue (est)', value: `$${(digest.adRevenueUsd ?? 0).toFixed(2)}` + fmtDelta(digest.adRevenueUsd, p.adRevenueUsd), inline: true },
    { name: 'Ad impressions', value: fmtNum(digest.adImpressions) + fmtDelta(digest.adImpressions, p.adImpressions), inline: true },
    { name: 'Adblock', value: fmtPct(digest.adblockPct), inline: true },
  ];
  let footer = 'UTC day · deltas vs prior day';
...
export async function start(token) {
  ...
  const sampleCcu = async () => {
    const now = Date.now();
    const results = [];
    for (const url of config.metrics.serverInfoUrls) {
      try {
        const info = await apiGet(url, { timeoutMs: 5000, retries: 0 });
        results.push({ url, count: Number(info.realPlayersCnt) || 0 });
      } catch {
        results.push({ url, count: null });
      }
    }
    ...
    for (const r of results) {
      if (r.count === null && rec.regionsSeen.includes(r.url)) rec.sawPartial = true;
    }
    if (sum > rec.peak) {
      rec.peak = sum;
      rec.atMs = now;
