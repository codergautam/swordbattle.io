...
class Chimera extends Entity {
    ...
    constructor(game, objectData) {
        ...
        this.angryTimer = new Timer(0, 4, 20);
        ...
        this.maxChargeSpeed = this.speed.value * 9.0;

        this.chargeAccel = 1400;
    ...
    applyVelocityKnockback(entity, mult = 1) {
        ...
        const force = (2 + speed) * 0.175 * mult;
    ...
    damaged(dmg, entity) {
        ...
        if (entity?.shape) {
            const dx = this.shape.x - entity.shape.x;
            const dy = this.shape.y - entity.shape.y;
            const dist = Math.sqrt(dx * dx + dy * dy) || 1;

            const kb = 10;
            this.velocity.x += (dx / dist) * kb;
            this.velocity.y += (dy / dist) * kb;
        }
