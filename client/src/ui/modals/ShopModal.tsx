...
const ShopModal: React.FC<ShopModalProps> = ({ account }) => {
  ...
  return (
    <div className="shop-modal">
      ...
      {account?.isLoggedIn ? (
      <h1 className='shop-desc'>Gems: {numberWithCommas(account.gems)}<img className={'gem'} src='assets/game/gem.png' alt='Gems' width={30} height={30} /><br />Snowtokens: {numberWithCommas(account.tokens)}<img className={'gem'} src='assets/game/snowtoken.png' alt='Snwotokens' width={30} height={30} /><br />Mastery: {numberWithCommas(account.mastery)}<img className={'gem'} src='assets/game/ultimacy.png' alt='Gems' width={30} height={30} /></h1>
       ) : (
...
<button onClick={scrollToTarget4} data-selected-badge="sale">Winter Skin Sale</button>
...
<button onClick={scrollToTarget2} data-selected-badge="ultimate">Ultimate Skins</button>
  ...
  skin?.buyable ? 'Free' : ''
      ...
      <div ref={targetElementRef3}></div>
        ...
        <p style={{color: 'white'}}>Earn <span style={{color: '#00ccff'}}>Snowtokens</span> from killing <span style={{color: '#ff8888ff'}}>mobs</span> and use them to unlock these special <span style={{color: '#00ffff'}}>event skins</span>! Some skins may become <span style={{color: '#ffff00'}}>exclusive</span> after this event ends and will no longer go on sale.</p>
        </div>
        <div className='skins'>
      {Object.values(skins).filter((skinData: any) => {
        const skin = skinData as Skin;
  ...
  <button className='buy-button' onClick={() => handleActionClick(skin.id)}>
    {skinStatus[skin.id] || (account.skins.equipped === skin.id ? 'Equipped' :
      ...
      <div ref={targetElementRef4}></div>
        <div className='label'>
        <span>Winter Skin Sale</span><hr></hr>
        <p style={{color: 'white'}}>Save up to <span style={{color: '#ffff00'}}>thousands</span> of gems on these skins during the winter event! These skins will also always be available to buy!</p>
        </div>
        <div className='skins'>
      {Object.values(skins).filter((skinData: any) => {
        const skin = skinData as Skin;
  ...
  <button className='buy-button' onClick={() => handleActionClick(skin.id)}>
    {skinStatus[skin.id] || (account.skins.equipped === skin.id ? 'Equipped' :
      ...
      {Object.values(skins).filter((skinData: any) => {
        ...
        if (skin.eventoffsale) return false;
        if ((skin.price ?? 0) <= 0) return false;
        if (!skin.buyable) return false;
        if ((skin.description || '').includes('Given')) return false;
        if (skin.currency) return false;
        if (skin.sale) return false;
        return skin.displayName.toLowerCase().includes(searchTerm.toLowerCase());
